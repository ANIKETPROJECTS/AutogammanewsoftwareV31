...
**How to apply:** Keep the token inside the managed connector. Configure the sender phone number ID separately and use it for both `/media` and `/messages` requests. Never place either credential in source code or chat.